...
## LXC
linux container：隔离进程和资源，不需要提供指令解释机制以及全虚拟化，没有准虚拟化和系统调用替换中的复杂性
# Docker
http://dockone.io/article/2941
- docker底层使用了LXC来实现
- 快速部署：基于镜像，不用进行完整的引导过程?
- 高效资源利用：不用硬件虚拟化，操作系统上的虚拟化，直接在cpu运行本地指令，访问设备
...
### 文件系统
- copy on write
- 挂载 CLONE_NEWNS创建隔离的挂载点命名空间。提供一个根文件系统（rootfs）
### CGroup
